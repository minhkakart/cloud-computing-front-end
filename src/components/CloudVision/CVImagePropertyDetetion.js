...
function CVImagePropertyDetetion() {
    ...
    const handleProcess = async (gcs_uri) => {
        ...
        await axios.get(process.env.REACT_APP_API_DOMAIN + "/api/csrf-cookie");
        const response = await axios.post(
            process.env.REACT_APP_API_DOMAIN + "/api/cloud-vision/image-property-detection",
            {
                gcs_uri,
            },
            {
        ...
        if (response.status === 200) {
            setImageProperty(response.data.properties);
        } else {
        ...
        <div>
            ...
            <div className="d-flex flex-column mt-4">
                <Card>
                    ...
                    <Card.Body>
                        <div className="row">
                            {imageProperty.length === 0 && <p>No result</p>}
                            {imageProperty.map((property, index)=>{
                                let bgr = `rgb(${property.red}, ${property.green}, ${property.blue})`;
                                return (
                                    <div key={index} style={{backgroundColor: bgr, height: '32px'}} className="col">
...
export default CVImagePropertyDetetion;
