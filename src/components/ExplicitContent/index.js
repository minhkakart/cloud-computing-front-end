...
function ExplicitContent() {
    ...
    const handleProcess = async (gcs_uri) => {
        ...
        await axios.get(process.env.REACT_APP_API_DOMAIN + "/api/csrf-cookie");
        const response = await axios.post(
            process.env.REACT_APP_API_DOMAIN + "/api/detect-explicit-content",
            {
                gcs_uri,
            },
            {
    ...
    return (
        <div>
            ...
            <div className="d-flex flex-column mt-4">
                <h2>Result</h2>
                <div>
                    {frames.length === 0 && <p>No result</p>}
                    {frames.map((frame, index) => {
                        return (
                            <div key={index}>
                                <p>Time offset: {frame.time}</p>
                                <p>Explicit content: {frame.pornography}</p>
                            </div>
                        );
                    })}
                </div>
